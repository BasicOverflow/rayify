...
3. **Nights 1..7** — cron schedules EOD → `jobs.dispatch` / `submit_loop`; quality via the same queue
4. End-of-week Serve / export / chaos hooks on that spine

Quality asserts: liquid pool symbols must finish with ``suspicious_count == 0`` (no ITMR-style ``linear_ramp`` / ``sparse_bridge``). Synthetic anomaly cases live under ``tests/unit/test_quality.py``.
