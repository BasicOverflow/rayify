...
Alternatively, you can use [`ray.runtime_env.RuntimeEnv`](api/doc/ray.runtime_env.RuntimeEnv.md#ray.runtime_env.RuntimeEnv "ray.runtime_env.RuntimeEnv"):
...
The `runtime_env` is a Python dictionary or a Python class [`ray.runtime_env.RuntimeEnv`](api/doc/ray.runtime_env.RuntimeEnv.md#ray.runtime_env.RuntimeEnv "ray.runtime_env.RuntimeEnv") including one or more of the following fields:
...
When specifying a path to a `requirements.txt` file, the file must be present on your local machine and it must be a valid absolute path or relative filepath relative to your local current working directory, _not_ relative to the `working_dir` specified in the `runtime_env`. Furthermore, referencing local files _within_ a `requirements.txt` file isn’t directly supported (e.g., `-r ./my-laptop/more-requirements.txt`, `./my-pkg.whl`). Instead, use the `${RAY_RUNTIME_ENV_CREATE_WORKING_DIR}` environment variable in the creation process. For example, use `-r ${RAY_RUNTIME_ENV_CREATE_WORKING_DIR}/my-laptop/more-requirements.txt` or `${RAY_RUNTIME_ENV_CREATE_WORKING_DIR}/my-pkg.whl` to reference local files, while ensuring they’re in the `working_dir`.
...
Note: `image_uri` is experimental. If you have some requirements or run into any problems, raise issues in [github](https://github.com/ray-project/ray/issues).
  ...
  * `config` (dict | [`ray.runtime_env.RuntimeEnvConfig`](api/doc/ray.runtime_env.RuntimeEnvConfig.md#ray.runtime_env.RuntimeEnvConfig "ray.runtime_env.RuntimeEnvConfig")): config for runtime environment. Either a dict or a RuntimeEnvConfig. Fields: (1) setup_timeout_seconds, the timeout of runtime environment creation, timeout is in seconds.
...
If an Actor or Task specifies a new `runtime_env`, it overrides the parent’s `runtime_env` (i.e., the parent Actor’s or Task’s `runtime_env`, or the Job’s `runtime_env` if Actor or Task doesn’t have a parent) as follows:
  ...
  * Every other field in the `runtime_env` will be _overridden_ by the child, not merged. For example, if `runtime_env["py_modules"]` is specified, it will replace the `runtime_env["py_modules"]` field of the parent.
...
Here is a list of different use cases and corresponding URLs:
  ...
  * Example: Retrieve package from a private GitHub repository using a Personal Access Token **during development**. **For production** see [this document](runtime_env_auth.md#runtime-env-auth) to learn how to authenticate private dependencies safely.
