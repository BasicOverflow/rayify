...
`Q(theta)` | **True reward function** we _want_ to optimize, but is not directly use for training. | **True generalization** – an theoretical and unobersvable in practice.  
...
A few other notes on the Tuner config:
  ...
  * `param_space` specifies the _initial_ `config` input to our training function. A `grid_search` over two values will launch two trials with a certain set of hyperparameters, and PBT will continue modifying them as training progresses.
