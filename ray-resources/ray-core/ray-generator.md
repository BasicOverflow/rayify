...
**Ray generator works with existing Ray APIs seamlessly**
  ...
  * Ray generators work with built-in [fault tolerance features](fault-tolerance.md#fault-tolerance) such as retry or lineage reconstruction.
...
[Fault tolerance features](fault-tolerance.md#fault-tolerance) work with Ray generator tasks and actor tasks. For example;
...
The [`ray.cancel()`](api/doc/ray.cancel.md#ray.cancel "ray.cancel") function works with both Ray generator tasks and actor tasks. Semantic-wise, cancelling a generator task isn’t different from cancelling a regular task. When you cancel a task, `next(gen)` can return the reference that contains `TaskCancelledError` without any special ordering guarantee.
